...
@onready var gun_pivot = $HeadNode/Camera3D/hand/hand/GunPivot
@onready var rifle = $HeadNode/Camera3D/hand/hand/GunPivot/rifle
@onready var shotgun = $HeadNode/Camera3D/hand/hand/GunPivot/shotgun
@onready var gun_anim = $HeadNode/Camera3D/hand/hand/GunPivot/rifle/AnimationPlayer
@onready var shotgun_anim = $HeadNode/Camera3D/hand/hand/GunPivot/shotgun/AnimationPlayer
@onready var gun_barrel = $HeadNode/Camera3D/hand/hand/GunPivot/rifle/RayCast3D
@onready var shotgun_barrel = $HeadNode/Camera3D/hand/hand/GunPivot/shotgun/RayCast3D
@onready var anim_tree = $AnimationPlayer
@onready var aim_ray = $Armature/Skeleton3D/hand/AimRay
@onready var aim_ray_end = $Armature/Skeleton3D/hand/AimRayEnd
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_pressed("sprint"):
		speed = SPRINT_SPEED
		if anim_tree.current_animation != "forward":
			anim_tree.set("parameters/conditions/run", true)
		if anim_tree.current_animation != "right":
			anim_tree.set("parameters/conditions/run", true)
		if anim_tree.current_animation != "left":
			anim_tree.set("parameters/conditions/run", true)
		if anim_tree.current_animation != "back":
			anim_tree.set("parameters/conditions/run_back", true)
	...
	if not Input.is_action_pressed("sprint"):
		...
		if anim_tree.current_animation != "left":
			anim_tree.set("parameters/conditions/walk", true)
		if anim_tree.current_animation != "back":
			anim_tree.set("parameters/conditions/walk_back", true)
	...
	if Input.is_action_pressed("rifle"):
		anim_tree.set("parameters/conditions/switch", true)
...
func _reload():
	...
	if current_mag == max_size or current_reserve <= 0:
		return
	is_reloading = true
	if Input.is_action_pressed("forward") or Input.is_action_pressed("left") or Input.is_action_pressed("right") or Input.is_action_pressed("back") and not Input.is_action_pressed("sprint"):
		anim_tree.set("parameters/conditions/reload_walk", true)
	elif Input.is_action_pressed("forward") or Input.is_action_pressed("left") or Input.is_action_pressed("right") or Input.is_action_pressed("back") and Input.is_action_pressed("sprint"):
		anim_tree.set("parameters/conditions/reload_run", true)
	elif anim_tree.current_animation != "idle":
		anim_tree.play("reload_idle")
	await get_tree().create_timer(2.0).timeout
